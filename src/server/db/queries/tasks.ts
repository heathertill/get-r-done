...
const all = () => knex('tasks').select('tasks.id', 'tasks.userid', 'users.name', 'tasks.title', 'tasks.details', 'categories.name as categoryname', 'priority.name as priorityname', 'tasks._created', 'tasks.due').join('categories', 'tasks.categoryid', '=', 'categories.id').join('priority', 'tasks.priorityid', '=', 'priority.id').join('users', 'tasks.userid', '=', 'users.id').orderBy('due', 'desc');
